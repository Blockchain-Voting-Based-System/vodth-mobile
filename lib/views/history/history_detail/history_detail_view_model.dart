...
class HistoryDetailViewModel extends BaseViewModel {
  ...
  Future<String> eventLiveVoteCount() async {
    ...
    SuiObjectResponse suiObjectResponse = await client.getObject(
        event?.suiEventId ?? '',
  ...
  Future<String> candidateLiveVoteCount(CandidateModel? candidate) async {
    ...
    candidate?.copyWith(
        voteCount: suiObjectResponse.data?.content?.fields['voted']);
  ...
  Future<void> updateAndSortCandidates() async {
    ...
    updatedCandidates.sort((a, b) =>
        int.parse(b.voteCount ?? '0').compareTo(int.parse(a.voteCount ?? '0')));
